...
For debugging and automated testing, you can enable console logging of all toast events. This is useful when working with browser automation tools that can capture console output.

```jsx
// Enable human-readable logs
<Toaster consoleLog />

// Enable JSON logs for automated tools
<Toaster consoleLog consoleLogFormat="json" />
```

You can also enable logging programmatically via the global config:

```jsx
// Enable before Toaster mounts
toast.configure({ enabled: true, format: 'json' });
```

Human-readable output:
```
[sonner:success] Payment processed (id: 3)
[sonner:error] Connection failed (id: 4, description: "Check your network")
...
{"event":"dismiss","id":3,"timestamp":1703...}
```
...
| consoleLog            |                                        Enables console logging of toast create and dismiss events                              |        `false` |
| consoleLogFormat      |                                            Console log format: `human` or `json`                                               |       `human` |
